    ...
    return (
        <>
            ...
            {inProgress ? (
                ...
                <>
                    <p>
                        Your group user has been created. To use spacetube on Matrix, copy the user id below and invite
                        it to your chat.
                    </p>
                ...
                <>
                    <p>
                        The first step to using spacetube. Your group user will send messages and perform actions on
                    ...
                    ></input>
                    <button onClick={createUser} disabled={!groupName} className="home-button">
                        Create
                    </button>
                </>
            )}
    ...
    if (!submitted)
        return (
            <>
                ...
                ></input>
                <button id="create-button" className="home-button" onClick={createInvite} disabled={!myName}>
                    Create
                </button>
            </>
        );
    ...
    if (submitted && link)
        return (
            <>
                ...
                <p>
                    Use <a href={`/?linkToken=${linkToken}&name=${myName}`}>this link</a> to view your side of the tube
                    once they've accepted the invite.
                </p>
                <CopyInput value={`${URL}/?linkToken=${linkToken}&name=${myName}`} type="my link" />
            </>
        );
};
    ...
    return (
        <>
            {errorMessage ? (
                ...
                linkToken ? (
                    <meta http-equiv="Refresh" content={`0; url='/?linkToken=${linkToken}&name=${myName}'`} />
                ) : (
                    <>
                        <p>Accepting invite, building rooms, inviting users, this takes a few seconds...</p>
                    </>
                )
            ) : (
                <>
                    <p>
                        {contactName} is inviting you to create a spacetube, connecting {contactGroupName} with your
                        group.
                    </p>
                    ...
                    ></input>
                    <button onClick={accept} className="home-button" disabled={!myName || !myGroupName}>
                        Accept Invite
                    </button>
                </>
            )}
    ...
    return (
        <div id="home-container">
            ...
            <div id="actions-container">
                <div className="form-container">{invite ? <InviteAccept invite={invite} /> : <GroupUserCreate />}</div>
                {linkTokens.length > 0 && (
